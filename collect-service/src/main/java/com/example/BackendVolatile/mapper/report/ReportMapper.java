...
public interface ReportMapper {
    ...
    @NotNull(message = ParamFormatErrorConstant.REPORT_DO_NOT_EXIST)
    @Select("SELECT task_id FROM reports WHERE report_id = #{ report_id }")
    Long get_task_id_by_report_id(@Param("report_id") Long report_id );

    @NotNull(message = ParamFormatErrorConstant.REPORT_DO_NOT_EXIST)
    @Select("SELECT report_state FROM reports WHERE report_id = #{ report_id }")
    Integer get_report_state_by_report_id(@Param("report_id") Long report_id );

    @NotNull(message = ParamFormatErrorConstant.REPORT_DO_NOT_EXIST)
    @Select("SELECT user_id FROM reports WHERE report_id = #{ report_id }")
    ...
    @Select("SELECT COUNT(1) FROM reports WHERE report_id = #{report_id}")
    ...
    @Select("SELECT COUNT(1) FROM reports WHERE report_id = #{report_id} AND task_id = #{task_id}")
    ...
    @Select("SELECT COUNT(1) FROM reports WHERE user_id = #{user_id} and task_id = #{task_id}")
    ...
    @Select("SELECT COUNT(1) FROM reports WHERE user_id = #{user_id} and report_id = #{report_id}")
    ...
    @NotNull(message = ParamFormatErrorConstant.NO_REPORT_EXIST)
    @Select("SELECT * FROM reports WHERE user_id = #{user_id} AND task_id = #{task_id}" )
    List<Report> get_all_by_user_id_and_task_id(
            @Param("user_id")Long user_id ,@Param("task_id")Long task_id);

    @NotNull(message = ParamFormatErrorConstant.NO_REPORT_EXIST)
    @Select("SELECT * FROM reports WHERE task_id = #{task_id}" )
    ...
    @Insert("INSERT INTO reports ( user_id, task_id, defect_explain,defect_reproduction_step," +
    ...
    @Update("UPDATE reports SET similarity = #{similarity} WHERE report_id = #{report_id}")
    void update_similarity_by_report_id(@Param("report_id") Long report_id, @Param("similarity") Integer similarity);

    @Update("UPDATE reports SET report_name = #{report.report_name}," +
    ...
    @Update("UPDATE reports SET similar_report_id = #{similar_report_id} WHERE report_id = #{report_id}")
    ...
    @NotNull(message = ParamFormatErrorConstant.NO_REPORT_EXIST)
    @Select("select MAX(report_id) from reports")
    Long max_id();
     ...
     */
    @NotNull(message = ParamFormatErrorConstant.NO_REPORT_EXIST)
    @Select("select count(*) from reports WHERE task_id = #{task_id};")
