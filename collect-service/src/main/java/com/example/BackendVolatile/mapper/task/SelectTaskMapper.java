...
public interface SelectTaskMapper {
    ...
    @Insert("INSERT INTO VR_task.select_task ( task_id ,user_id) " +
            "VALUES (#{taskId}, #{userId}) ")
    void insert( @Param("taskId") Long taskId,@Param("userId") Long userId );

    @NotNull
    @Select("SELECT count(1) FROM VR_task.select_task where task_id=#{taskId} AND user_id=#{userId} ")
    ...
    @Select("SELECT task_id FROM VR_task.select_task WHERE user_id = #{userId}")
    List<Long> select_task_id_by_userid(@Param("userId") Long userId);


    @Select("SELECT user_id FROM VR_task.select_task WHERE task_id = #{task_id}")
